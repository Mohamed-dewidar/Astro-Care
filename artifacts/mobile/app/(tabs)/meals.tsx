...
import { FOOD_UNITS, MEAL_CATEGORY_LABELS, MealCategory, MealTemplate } from "@/types";
...
export default function MealsScreen() {
  ...
  const {
    todayMeals, foods, mealTemplates,
    completeMeal, skipMeal, addMeal,
    addFood, deleteFood, toggleFavoriteFood,
  ...
  const [newMealCategory, setNewMealCategory] = useState<MealCategory>("breakfast");
  ...
  const [schedulingTemplate, setSchedulingTemplate] = useState<MealTemplate | null>(null);
  ...
  const sortedMeals = useMemo(
    () => [...todayMeals].sort((a, b) => a.scheduledTime.localeCompare(b.scheduledTime)),
    [todayMeals]
  );
  ...
  const filteredFoods = useMemo(() =>
    !searchQuery ? foods : foods.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase())),
    [foods, searchQuery]
  );

  const pickerFilteredFoods = useMemo(() =>
    !foodPickerSearch ? foods : foods.filter(f => f.name.toLowerCase().includes(foodPickerSearch.toLowerCase())),
    [foods, foodPickerSearch]
  );

  const editPickerFoods = useMemo(() =>
    !editFoodSearch ? foods : foods.filter(f => f.name.toLowerCase().includes(editFoodSearch.toLowerCase())),
    [foods, editFoodSearch]
  );

  const canAddMeal = newMealName.trim().length > 0 && selectedFoodIds.length > 0;
  const canSaveEdit = editName.trim().length > 0 && editFoodIds.length > 0;

  // ── Handlers ──────────────────────────────────────────────────────────────
  const closeFoodModal = () => {
    setAddFoodModal(false);
    setNewFoodName(""); setNewFoodCal("");
    fabRef.current?.close();
  };

  const closeMealModal = () => {
    setAddMealModal(false);
    setNewMealName(""); setNewMealTime("08:00");
    setNewMealCategory("breakfast"); setSelectedFoodIds([]); setFoodPickerSearch("");
    fabRef.current?.close();
  };

  const handleAddFood = () => {
    if (!newFoodName.trim()) return;
    addFood({ name: newFoodName.trim(), calories: newFoodCal ? Number(newFoodCal) : undefined });
    closeFoodModal();
  ...
  const handleAddMeal = () => {
    ...
    addMeal({
      name: newMealName.trim(), category: newMealCategory,
      scheduledTime: newMealTime, reminderEnabled: true,
  ...
  const toggleFoodSel = (foodId: string) => {
    setSelectedFoodIds(prev => prev.includes(foodId) ? prev.filter(id => id !== foodId) : [...prev, foodId]);
  ...
  const closeEdit = () => {
    ...
    setEditName(""); setEditFoodIds([]); setEditFoodSearch("");
  };
  ...
  const toggleEditFood = (foodId: string) => {
    setEditFoodIds(prev => prev.includes(foodId) ? prev.filter(id => id !== foodId) : [...prev, foodId]);
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
  };
  ...
  const openSchedule = (t: MealTemplate) => {
    ...
    setScheduleTime(t.category === "breakfast" ? "08:00" : t.category === "lunch" ? "13:00" : t.category === "dinner" ? "19:00" : "10:00");
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>{sortedMeals.length} orbits scheduled today</Text>
        </View>
            ...
            {(["today", "foods", "meals"] as TabType[]).map(tab => (
              ...
              >
                ...
                <Text style={[styles.tabText, activeTab === tab && styles.activeTabText]}>
                  {tab === "today" ? "Today" : tab === "foods" ? "Foods" : "Meals"}
                </Text>
              </Pressable>
            ))}
          </GlassCard>
          ...
          <View>
            {sortedMeals.length === 0 ? (
              <View style={styles.empty}>
                ...
                <Text style={styles.emptyText}>Add meals to start your health mission</Text>
              </View>
            ) : (
              ...
              filteredFoods.map(food => (
                <GlassCard key={food.id} style={styles.foodCard}>
                  <View style={styles.foodRow}>
                    <View style={[styles.foodIcon, { backgroundColor: food.isFavorite ? "rgba(251,191,36,0.15)" : "rgba(124,58,237,0.1)" }]}>
                      <Ionicons name="nutrition" size={20} color={food.isFavorite ? "#FBBF24" : "#7C3AED"} />
                    </View>
                    <View style={styles.foodInfo}>
                      <Text style={styles.foodName}>{food.name}</Text>
                      {food.calories !== undefined && (
                        <Text style={styles.foodMacros}>{food.calories} kcal{food.protein !== undefined ? ` · ${food.protein}g protein` : ""}</Text>
                      )}
                    </View>
                    <View style={styles.foodActions}>
                      <Pressable onPress={() => toggleFavoriteFood(food.id)} style={styles.foodActionBtn}>
                        <Ionicons name={food.isFavorite ? "star" : "star-outline"} size={18} color={food.isFavorite ? "#FBBF24" : "#475569"} />
                      </Pressable>
                      <Pressable
                        onPress={() => confirmDelete("Delete Food", `Delete "${food.name}"?`, () => deleteFood(food.id))}
              ...
              mealTemplates.map(t => {
                ...
                return (
                  <GlassCard key={t.id} style={styles.tplCard}>
                    ...
                    <View style={styles.tplHeader}>
                      ...
                      <View style={styles.tplInfo}>
                        ...
                        <Text style={styles.tplMeta}>
                          {MEAL_CATEGORY_LABELS[t.category]} · {foodCount} food{foodCount !== 1 ? "s" : ""}
                        </Text>
                      </View>
                      <Pressable
                        onPress={() => confirmDelete("Delete Meal", `Remove "${t.name}" from library?`, () => deleteMealTemplate(t.id))}
                        hitSlop={8}
                      >
                    ...
                    {/* Food chips */}
                    {t.items.length > 0 && (
                      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tplFoodScroll}>
                        {t.items.map(item => (
                          <View key={item.id} style={styles.tplFoodChip}>
                            <Text style={styles.tplFoodChipText}>{foodMap[item.foodId] ?? "—"}</Text>
                          </View>
                        ))}
                      </ScrollView>
                    )}
                    ...
                    <View style={styles.tplActions}>
                      <Pressable style={styles.tplEditBtn} onPress={() => openEdit(t)}>
                        <Ionicons name="pencil" size={14} color="#94A3B8" />
                        <Text style={styles.tplEditBtnText}>Edit</Text>
                      </Pressable>
                      <Pressable style={styles.tplScheduleBtn} onPress={() => openSchedule(t)}>
                        <LinearGradient
                          colors={["#7C3AED", "#3B82F6"]}
                          start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                        ...
                        >
                          ...
                          <Text style={styles.tplScheduleBtnText}>Schedule</Text>
                        </LinearGradient>
                      </Pressable>
                    </View>
                  </GlassCard>
      ...
      <FloatingActionButton
        ref={fabRef}
        actions={[
          { icon: "restaurant", label: "Add Meal", color: "#7C3AED", onPress: () => setAddMealModal(true) },
          { icon: "nutrition", label: "Add Food", color: "#22D3EE", onPress: () => setAddFoodModal(true) },
        ]}
      />

      {/* ════ Add Food Modal ════ */}
      <Modal visible={addFoodModal} transparent animationType="slide" onRequestClose={closeFoodModal}>
        <Pressable style={styles.modalOverlay} onPress={closeFoodModal}>
          <Pressable>
            <GlassCard style={[styles.modalCard, { paddingBottom: insets.bottom + 16 }]}>
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>New Food</Text>
                <Pressable onPress={closeFoodModal} style={styles.closeBtn} hitSlop={8}>
                  <Ionicons name="close" size={20} color="#94A3B8" />
                </Pressable>
              </View>
              <TextInput style={styles.input} placeholder="Food name *" placeholderTextColor="#475569" value={newFoodName} onChangeText={setNewFoodName} />
              <TextInput style={styles.input} placeholder="Calories (optional)" placeholderTextColor="#475569" value={newFoodCal} onChangeText={setNewFoodCal} keyboardType="numeric" />
              <Pressable style={styles.modalBtn} onPress={handleAddFood}>
                <LinearGradient colors={["#7C3AED", "#3B82F6"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.modalBtnGradient}>
      ...
      {/* ════ Add Meal (today) Modal ════ */}
      <Modal visible={addMealModal} transparent animationType="slide" onRequestClose={closeMealModal}>
        <Pressable style={styles.modalOverlay} onPress={closeMealModal}>
          <Pressable style={styles.bigModalWrapper}>
            <GlassCard style={[styles.bigModalCard, { paddingBottom: insets.bottom + 16 }]}>
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>Schedule Meal</Text>
                <Pressable onPress={closeMealModal} style={styles.closeBtn} hitSlop={8}>
                  <Ionicons name="close" size={20} color="#94A3B8" />
                </Pressable>
              </View>

              <TextInput style={styles.input} placeholder="Meal name *" placeholderTextColor="#475569" value={newMealName} onChangeText={setNewMealName} />
              <TimePicker value={newMealTime} onChange={setNewMealTime} label="Scheduled Time" />
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryScroll}>
                {ALL_CATEGORIES.map(cat => (
                  <Pressable
                    ...
                    style={[styles.categoryChip, newMealCategory === cat && styles.categoryChipActive]}
                  >
                    <Text style={[styles.categoryChipText, newMealCategory === cat && styles.categoryChipTextActive]}>
              ...
              <Pressable style={[styles.modalBtn, !canAddMeal && styles.modalBtnDisabled]} onPress={handleAddMeal} disabled={!canAddMeal}>
                <LinearGradient
                  colors={canAddMeal ? ["#7C3AED", "#3B82F6"] : ["#1E293B", "#1E293B"]}
                  start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                  style={styles.modalBtnGradient}
                >
                  <Ionicons name="rocket" size={16} color={canAddMeal ? "#FFFFFF" : "#475569"} />
                  <Text style={[styles.modalBtnText, !canAddMeal && styles.modalBtnTextDisabled]}>
                    {!newMealName.trim() ? "Enter a meal name" : selectedFoodIds.length === 0 ? "Select at least one food" : "Launch Mission"}
                  </Text>
                </LinearGradient>
              </Pressable>
            </GlassCard>
          ...
          <Pressable style={styles.bigModalWrapper}>
            <GlassCard style={[styles.bigModalCard, { paddingBottom: insets.bottom + 16 }]}>
              <View style={styles.modalHeader}>
                ...
                <Pressable onPress={closeEdit} style={styles.closeBtn} hitSlop={8}>
                  <Ionicons name="close" size={20} color="#94A3B8" />
                </Pressable>
              </View>

              <TextInput style={styles.input} placeholder="Meal name *" placeholderTextColor="#475569" value={editName} onChangeText={setEditName} />
              ...
              <Pressable style={[styles.modalBtn, !canSaveEdit && styles.modalBtnDisabled]} onPress={saveEdit} disabled={!canSaveEdit}>
                <LinearGradient
                  colors={canSaveEdit ? ["#7C3AED", "#3B82F6"] : ["#1E293B", "#1E293B"]}
                  start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                  style={styles.modalBtnGradient}
                >
                  <Ionicons name="checkmark" size={16} color={canSaveEdit ? "#FFF" : "#475569"} />
                  <Text style={[styles.modalBtnText, !canSaveEdit && styles.modalBtnTextDisabled]}>Save Changes</Text>
                </LinearGradient>
              </Pressable>
            </GlassCard>
          </Pressable>
      ...
      {/* ════ Schedule from Template Modal ════ */}
      <Modal visible={scheduleModal} transparent animationType="slide" onRequestClose={closeSchedule}>
            ...
            <GlassCard style={[styles.modalCard, { paddingBottom: insets.bottom + 16 }]}>
              <View style={styles.modalHeader}>
                <View>
                  ...
                  {schedulingTemplate && (
                    <Text style={styles.scheduleSubtitle}>{schedulingTemplate.name}</Text>
                  )}
                </View>
                <Pressable onPress={closeSchedule} style={styles.closeBtn} hitSlop={8}>
                  <Ionicons name="close" size={20} color="#94A3B8" />
                </Pressable>
              </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.dayScroll}>
                {SCHEDULE_DAYS.map(d => (
                  <Pressable
                    key={d.value}
                    onPress={() => { setScheduleDate(d.value); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
                    style={[styles.dayChip, scheduleDate === d.value && styles.dayChipActive]}
                  >
                    <Text style={[styles.dayChipText, scheduleDate === d.value && styles.dayChipTextActive]}>
                      {d.label}
                    </Text>
                  </Pressable>
                ))}
...
function FoodPickerSection({ label, required, foods, filteredFoods, selectedIds, foodMap, search, onSearchChange, onToggle }: FoodPickerSectionProps) {
  return (
    <View style={styles.foodPickerSection}>
      <View style={styles.foodPickerHeader}>
        <Text style={styles.foodPickerLabel}>
          {label}
          {required && <Text style={styles.foodPickerRequired}> * required</Text>}
        </Text>
        {selectedIds.length > 0 && (
          <View style={styles.selectedBadge}>
            <Text style={styles.selectedBadgeText}>{selectedIds.length} selected</Text>
          </View>
        )}
      </View>
      ...
      {foods.length === 0 ? (
        <View style={styles.pickerEmpty}>
          ...
          <Text style={styles.pickerEmptyText}>No foods yet — add some in the Foods tab first</Text>
        </View>
      ) : (
        <ScrollView style={styles.pickerList} showsVerticalScrollIndicator={false} nestedScrollEnabled>
          {filteredFoods.map(food => {
            ...
            return (
              ...
              >
                <View style={[styles.pickerFoodIcon, { backgroundColor: isSelected ? "rgba(124,58,237,0.2)" : "rgba(255,255,255,0.04)" }]}>
                  <Ionicons name="nutrition" size={16} color={isSelected ? "#A78BFA" : "#475569"} />
                </View>
                <View style={styles.pickerFoodInfo}>
                  <Text style={[styles.pickerFoodName, isSelected && styles.pickerFoodNameSelected]}>{food.name}</Text>
                  {food.calories !== undefined && <Text style={styles.pickerFoodMeta}>{food.calories} kcal</Text>}
                </View>
...
const styles = StyleSheet.create({
  ...
  tab: { flex: 1, paddingVertical: 10, alignItems: "center", borderRadius: 12, overflow: "hidden" },
  activeTab: {},
  tabText: { color: "#64748B", fontSize: 13, fontWeight: "600" },
  activeTabText: { color: "#FFFFFF" },
  // Foods tab
  searchBar: { flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 14, paddingVertical: 12, marginBottom: 12 },
  ...
  foodIcon: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center" },
  foodInfo: { flex: 1 },
  ...
  tplHeader: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 10 },
  tplIconWrap: { width: 38, height: 38, borderRadius: 19, backgroundColor: "rgba(124,58,237,0.12)", alignItems: "center", justifyContent: "center" },
  tplInfo: { flex: 1 },
  ...
  tplScheduleGrad: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 10 },
  ...
  scheduleSubtitle: { color: "#A78BFA", fontSize: 13, fontWeight: "500", marginTop: 2 },
  ...
  categoryChipActive: { backgroundColor: "rgba(124,58,237,0.25)", borderColor: "#7C3AED" },
  categoryChipText: { color: "#64748B", fontSize: 13, fontWeight: "500" },
  categoryChipTextActive: { color: "#A78BFA", fontWeight: "600" },
  sectionLabel: { color: "#94A3B8", fontSize: 11, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 },
  ...
  dayChipActive: { backgroundColor: "rgba(124,58,237,0.25)", borderColor: "#7C3AED" },
  ...
  modalBtnGradient: { paddingVertical: 16, alignItems: "center", flexDirection: "row", justifyContent: "center", gap: 8 },
  modalBtnText: { color: "#FFFFFF", fontSize: 16, fontWeight: "700" },
  modalBtnTextDisabled: { color: "#475569" },
  // Food picker (shared)
  foodPickerSection: { marginBottom: 4 },
  foodPickerHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 },
  foodPickerLabel: { color: "#94A3B8", fontSize: 13, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.6 },
  foodPickerRequired: { color: "#EF4444", fontSize: 11, fontWeight: "500", textTransform: "none", letterSpacing: 0 },
  selectedBadge: { backgroundColor: "rgba(124,58,237,0.2)", borderRadius: 10, paddingHorizontal: 10, paddingVertical: 3, borderWidth: 1, borderColor: "rgba(124,58,237,0.4)" },
  ...
  pickerEmptyText: { color: "#475569", fontSize: 13, textAlign: "center", lineHeight: 18 },
  pickerRow: {
    flexDirection: "row", alignItems: "center", gap: 10,
    paddingHorizontal: 12, paddingVertical: 10, borderRadius: 10, marginBottom: 4,
    backgroundColor: "rgba(255,255,255,0.03)", borderWidth: 1, borderColor: "rgba(255,255,255,0.06)",
  },
  pickerRowSelected: { backgroundColor: "rgba(124,58,237,0.12)", borderColor: "rgba(124,58,237,0.35)" },
  pickerFoodIcon: { width: 30, height: 30, borderRadius: 15, alignItems: "center", justifyContent: "center" },
  ...
  checkCircle: { width: 20, height: 20, borderRadius: 10, borderWidth: 1.5, borderColor: "rgba(255,255,255,0.15)", alignItems: "center", justifyContent: "center" },
