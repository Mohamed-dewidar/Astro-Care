...
function rowToMeal(r: MealRow): ScheduledMeal {
  return {
    id: r.id,
    name: r.name,
    category: r.category as ScheduledMeal["category"],
...
export function dbGetMeals(): ScheduledMeal[] {
  return getDb()
    .getAllSync<MealRow>(
      "SELECT * FROM meals ORDER BY date, scheduled_time",
      [],
    )
    .map(rowToMeal);
}
...
function rowToMed(r: MedRow): Medication {
  return {
    ...
    computedTime: r.computed_time ?? undefined,
    completedAt: r.completed_at ?? undefined,
    skipped: r.skipped === 1,
    date: r.date,
  };
...
export function dbGetMedications(): Medication[] {
  return getDb()
    .getAllSync<MedRow>(
      "SELECT * FROM medications ORDER BY date, computed_time",
      [],
    )
    .map(rowToMed);
}
...
export function dbGetMealTemplates(): MealTemplate[] {
  return getDb()
    ...
    .map((r) => ({
      id: r.id,
      name: r.name,
      category: r.category as MealTemplate["category"],
      items: JSON.parse(r.items || "[]"),
      colorTag: r.color_tag ?? undefined,
      isFavorite: r.is_favorite === 1,
    }));
